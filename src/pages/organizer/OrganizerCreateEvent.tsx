  ...
  return (
    <div className="min-h-[calc(100dvh-4rem)] bg-background text-foreground">
      <OrganizerWizardHeader
        ...
        isDisabled={w.isLoadingEditingEvent || Boolean(w.loadError)}
      />

      <div className="px-4 pt-32 pb-10 md:px-6 lg:px-10 xl:px-11">
        {w.isLoadingEditingEvent ? (
          <div className="mx-auto max-w-[1434px] rounded-lg border border-border bg-surface-secondary p-8 text-center text-sm text-muted">
            Loading event...
          </div>
        ) : w.loadError ? (
          <div className="mx-auto max-w-[1434px] rounded-lg border border-danger/40 bg-danger/10 p-8 text-center text-sm font-medium text-danger">
            {w.loadError}
          </div>
        ) : (
        ...
        {w.submitError && (
          <div className="mx-auto mt-4 max-w-[1434px] rounded-lg border border-danger/40 bg-danger/10 px-4 py-3 text-sm font-medium text-danger">
            {w.submitError}
          </div>
        )}
      </div>
