...
import { Button, TextField, Label, Input, FieldError, Form } from "@heroui/react";
  ...
  return (
    <div className="flex flex-col h-[100dvh] w-full bg-[#0a0a0a] text-white font-sans overflow-hidden">
      {/* ╔═══════════════════════════════╗
         ║       TOP HEADER BAR          ║
         ╚═══════════════════════════════╝ */}
      <header className="shrink-0 flex items-center justify-between px-4 md:px-8 py-3 bg-[#111] border-b border-white/5">
        {/* Logo / Back */}
        <div className="flex items-center gap-3">
          <button
            onClick={() => navigate(`/events/${event.id}/booking`)}
            className="flex items-center gap-2 text-(--accent) hover:text-(--accent)/80 font-semibold transition-colors text-sm"
          >
            <ArrowLeft size={18} />
            <span className="hidden md:inline">{t("common.back")}</span>
          </button>
          <div className="hidden md:block h-5 w-px bg-white/15" />
          <span className="text-lg font-extrabold tracking-tight text-(--accent) hidden md:block">
            TicketRush
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 text-sm">
          <Clock
            size={16}
            className={expired ? "text-red-400" : "text-(--accent)"}
          />
          <span className="text-white/50 hidden md:inline text-xs">
            {t("payment.holdTime")}
          </span>
          <div
        ...
        <div className="max-w-6xl mx-auto w-full p-4 md:p-8 lg:p-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-6">
              ...
              <Form
                className="bg-[#1a1a1a] rounded-xl border border-white/5 p-6 space-y-5"
